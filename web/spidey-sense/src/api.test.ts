...
describe('coordinate report adapter', () => {
  it('maps dynamic plan, sessions, and Graph evidence into dashboard data', () => {
    const snapshot = toDashboardSnapshot({
      ...
      graph: { provider: 'entire-graph', provider_version: 'test', commit: '1234567890', profile: 'full', completeness_level: 'complete', node_count: 4, relation_count: 1 },
      ...
      decisions: [{
        ...
        path: [{ from: { id: 'one', name: 'One', kind: 'function', file: 'one.go', start_line: 2 }, to: { id: 'two', name: 'Two', kind: 'function', file: 'two.go', start_line: 3 }, relation: 'CALLS', confidence: 1, resolution: 'exact' }],
        recommendations: ['Sequence missions.'], test_targets: ['one_test.go'],
      }],

...
var commandDocs = []commandDoc{
	...
	},
	{
		...
		usage:   []string{"entire graph coordinate --plan <file.json> [--repo path] [--head] [--format text|json] [--listen host:4317] [--allow-remote] [--network-state path]"},
		...
		flags: []flagDoc{
			...
			{name: "--listen", arg: "host:port", desc: "Serve the report and team-connectivity API"},
			{name: "--allow-remote", desc: "Explicitly permit a non-loopback bind for LAN or deployed use"},
			{name: "--network-state", arg: "path", desc: "Persist hashed team credentials and bounded agent state"},
		},
		examples: []string{"entire graph coordinate --repo . --plan spidey-plan.json --format text", "entire graph coordinate --repo . --plan spidey-plan.json --listen 0.0.0.0:4317 --allow-remote --network-state /var/lib/spidey/state.json"},
	},
	{
		name:    "connect-agent",
		group:   groupAnalyze,
		summary: "Connect this Entire session to a shared Spidey Sense team",
		usage:   []string{"entire graph connect-agent --server <url> --team <id> --invite <code> --name <name> [--role role] [--mission id] [--once]"},
		long:    "Joins a shared team, registers the current Entire session, and sends allowlisted heartbeats containing agent/provider identity, mission, branch, changed file paths, latest checkpoint, blocker, and last activity. Prompt text, reasoning, terminal content, file content, and secrets are never decoded into the heartbeat contract.",
		flags: []flagDoc{
			{name: "--server", arg: "url", desc: "Reachable shared backend URL (HTTPS unless loopback or explicitly allowed for LAN)"},
			{name: "--team", arg: "id", desc: "Team ID returned by POST /api/v1/teams"},
			{name: "--invite", arg: "code", desc: "Invite code supplied by the leader"},
			{name: "--name", arg: "name", desc: "User-entered teammate name"},
			{name: "--role", arg: "role", desc: "User-entered teammate role"},
			{name: "--mission", arg: "id", desc: "Current mission ID"},
			{name: "--blocker", arg: "text", desc: "Explicit bounded blocker status"},
			{name: "--interval", arg: "duration", def: "15s", desc: "Heartbeat interval (minimum 5s)"},
			{name: "--once", desc: "Register and send one heartbeat, then exit"},
			{name: "--allow-insecure-http", desc: "Allow plain HTTP to a non-loopback LAN server"},
		},
		examples: []string{"entire graph connect-agent --server https://spidey.example --team team_x --invite invite_x --name 'Deepthi' --role 'UI engineer' --mission connection-ui"},
	},
	{
		name:    "verify",
		group:   groupAnalyze,
